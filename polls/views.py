...
def polls_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid:
            ...
            messages.success(
                request, "New Poll Question Added Successfully!", extra_tags='alert alert-success alert-dismissible fade show')
...
def polls_edit(request, poll_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid:
            form.save()
            messages.success(request, "Poll Question Updated successfully!", extra_tags='alert alert-success alert-dismissible fade show')
            return redirect("polls:list")
...
def poll_detail(request, poll_id):
    ...
    if not poll.user_can_vote(request.user):
        return render(request, 'polls/poll_result.html', {'poll': poll})
    choice_count = poll.choice_set.count()
    context = {
        'poll': poll,
        'loop_time': range(0, choice_count),
    }
...
@login_required
def get_chart_data(request, poll_id):
    votes_data = []
    poll = get_object_or_404(Poll, pk=poll_id)
    votes = poll.choice_set.all()

    for i in votes:
        votes_data.append({i.choice_text:i.get_vote_count})
